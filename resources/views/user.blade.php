...
<div class = "selectyourrole shadow p-3 mb-5 rounded">
        ...
        <div class="teacher rounded-left">
            ...
            <form action="{{ route ('UserUp') }}" method = "GET">
                @csrf
                <input type="submit" class="btn btn-login btn-lg btn-block rounded-pill" name = "choose" value = "Empieza como docente">
            </form>
        </div>
        ...
        <div class="student rounded-right" >
             <p class="h4 text-center">{{ __('¿Quieres consultar y aprender a la marcha de trabajar?') }}</p><br>
                ...
                <form action=" {{ route ('UserUp') }} " method = "GET">
                    @csrf
                    <input type = "submit" class="btn btn-login btn-lg btn-block rounded-pill" name = "choose" value = "Empieza como alumno">
                </form>
        </div>
